...
resource "azurerm_virtual_machine_extension" "db" {
  name                 = "db"
  virtual_machine_id   = azurerm_linux_virtual_machine.vms[2].id
...
resource "azurerm_virtual_machine_extension" "nodes" {
  ...
  settings = <<SETTINGS
  {
    ...
    "commandToExecute": "sh nodes.sh ${data.azurerm_key_vault_secret.passwords[0].content_type} ${azurerm_network_interface.nic[2].private_ip_address}"
  }
  SETTINGS
